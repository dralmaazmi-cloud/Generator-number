...
function samePriceGainAndLoss(ctx) {
  ...
  return buildBase(ctx, {
    ...
    correct, distractors, format: unitFormat('dirham'),
    steps: [
      `في القطعة الرابحة السعر = التكلفة + ${g}% من التكلفة، أي ${100} + ${g} = ${u(100 + g, 'part')} من كل 100.`,
      `تكلفة القطعة الرابحة = ${price} × 100 ÷ ${100 + g} = ${costGain}.`,
      `وفي القطعة الخاسرة السعر = التكلفة − ${g}% من التكلفة، أي ${100} − ${g} = ${u(100 - g, 'part')} من كل 100.`,
