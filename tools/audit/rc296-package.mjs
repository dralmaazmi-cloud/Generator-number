#!/usr/bin/env node
// RC2.9.6 §6. The delivery package (RC2.9.5 §9's, carried forward).
//
...
const OUT = 'RC2_9_6_GENERATOR_READY.zip';
const STAGE = '.rc296-package';
...
// RC2.9.6 §6. The earlier releases' production bundles — now rc295 as well as
// rc294, rc293 and rc292 — so a reviewer can compare on matched seeds instead
// of taking this release's word for what changed. RC2.9.6 needs rc295 in
// particular: §4.4's frozen-surface table is a diff against it.
...
writeFileSync(join(STAGE, 'RELEASE.json'), JSON.stringify({
  ...
  contains: 'the engine as frozen, its tests, its audit tools, this release\'s evidence, and the RC2.9.5, RC2.9.4, RC2.9.3 and RC2.9.2 production bundles under rc2/baseline/',
