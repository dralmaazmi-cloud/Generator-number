#!/usr/bin/env node
// RC2.4 — the reveal key for Holdout F.
//
// A projection, not a generation: the only input is the file written at the
// first and only generation of AUDIT-2026-09-13-F. Nothing here imports the
...
const manifest = JSON.parse(readFileSync(join(ROOT, 'rc2/HOLDOUT_F.json'), 'utf8'));
const fullText = gunzipSync(readFileSync(join(ROOT, 'rc2/holdout-f-full.jsonl.gz'))).toString('utf8');
