#!/usr/bin/env node
// RC2.4 — Holdout E, the sign-off holdout, on a seed no run has ever touched.
//
...
export const HOLDOUT_SEED = 'AUDIT-2026-09-13-E';
export const OUT = 'rc2/HOLDOUT_E.json';
export const OUT_BLIND = 'rc2/holdout-e-blind.jsonl.gz';
export const OUT_FULL = 'rc2/holdout-e-full.jsonl.gz';
 ...
 * AUDIT-2026-09-13-E. The real seed is the default, the CLI never passes
 ...
 */
export function buildHoldoutE({seed = HOLDOUT_SEED, force = false} = {}) {
  const isRealHoldout = seed === HOLDOUT_SEED;
  if (isRealHoldout && !force && existsSync(OUT)) {
    throw Object.assign(
      new Error(`HOLDOUT_ALREADY_GENERATED: ${OUT} exists. Holdout E is generated exactly once and is never regenerated.`),
  ...
  batch.sessions.forEach((s, i) => {
    ...
    s.questions.forEach((q, k) => {
      n++;
      const itemId = `E-S${i + 1}-${String(k + 1).padStart(2, '0')}`;
  ...
  return {
    report: {
      schema: 'rc24-holdout-v1',
      section: 'RC2.4 sign-off holdout',
      ...
      frozenAt: {
        RC2_4_COMMIT: freeze.RC2_COMMIT, treeHash: freeze.treeHash,
      ...
      totals: {
        ...
        // Compared band by band. An earlier version compared the two objects
        // with JSON.stringify, which compares KEY ORDER as well as counts, so a
        // delivered tally that happened to be built in a different order read as
        // a mismatch while every count was identical. The preserved HOLDOUT_E
        // manifest carries that false negative; see rc2/HOLDOUT_E_CORRECTIONS.json.
      ...
      declaration: 'Generated once from the frozen RC2.4 engine. Not to be inspected or remediated before independent review.'
    },
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const {report, blindGz, fullGz} = buildHoldoutE();
