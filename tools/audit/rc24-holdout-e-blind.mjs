#!/usr/bin/env node
// RC2.4 — the blind-review artifacts for AUDIT-2026-09-13-E.
//
...
//   2. it opens rc2/holdout-e-blind.jsonl.gz and NOTHING else. The full file
...
//
// One difference from the Holdout D package: that one printed the template id
// and family beside each question. The RC2.4 brief puts metadata out of the
// blind package, and the RC2.4 blind dataset does not carry those fields at all
// — so there is nothing here to filter, which is the stronger arrangement.
//
...
const SOURCE = 'rc2/holdout-e-blind.jsonl.gz';
const OUT = 'rc2/holdout-e-blind-package';
const ZIP = 'rc2/RC2_4_HOLDOUT_E_BLIND.zip';
...
function text() {
  const out = [
    'SIGN-OFF HOLDOUT — AUDIT-2026-09-13-E',
...
${body}
<footer>${esc(sessionId)} — ${sel.length} questions — blind — AUDIT-2026-09-13-E</footer>
...
const scratch = process.env.RC24_SCRATCH || '/tmp/rc24-holdout-e-html';
...
const leaks = ['correctOption', 'correctValue', 'misconception', 'derivation', 'semanticFingerprint',
  'fingerprint', 'complexity', 'telemetry', 'explanation', 'templateId', 'RC2.4', 'REMEDIATION']
