...
/**
 * RC2.9.3, on the same seeds again: this release changes what the renderer
 * prints (explanations, rate words, a few stems and option pools), not which
 * question a seed draws, so the two corpora read line for line.
 */
export const RC293_DEVELOPMENT_SEEDS = RC292_DEVELOPMENT_SEEDS;

/**
 * RC2.9.3 names its sign-off holdout and does not generate it; its brief
 * forbids generating one. RC2.9.2's K is spent by being frozen against.
 */
export const RC293_SIGNOFF_SEED = 'AUDIT-2026-09-13-L';

/**
 * RC2.9.4, on the same seeds again. Phase A of this release changed nothing a
 * seed draws (proved by rc2/RC294_ZERO_DIFF.json); Phase B added twenty-nine
 * EASY and MEDIUM constructions and re-planned single-band sittings, so the
 * corpus on these seeds is the record of what this engine draws now.
 */
export const RC294_DEVELOPMENT_SEEDS = RC293_DEVELOPMENT_SEEDS;

/**
 * RC2.9.4 names its sign-off holdout and does not generate it; its brief
 * forbids generating one. RC2.9.3's L is spent by being frozen against.
 */
...
/**
 * RC2.9.6, on the same seeds again. Four parameter draws were constrained
 * (§3.1) and the in-sitting control was tightened (§3.4), so what these seeds
 * draw is not quite what RC2.9.5 drew; rc2/RC296_FROZEN_SURFACE.json says
 * exactly which 47 of 10,000 moved and which template moved each.
 */
export const RC296_DEVELOPMENT_SEEDS = RC295_DEVELOPMENT_SEEDS;

/**
 * RC2.9.6 names its sign-off holdout and does not generate it. RC2.9.5's N is
 * spent by being frozen against.
 */
export const RC296_SIGNOFF_SEED = 'AUDIT-2026-09-13-P';
...
export const RELEASES = Object.freeze({
  ...
  rc296: {seeds: RC296_DEVELOPMENT_SEEDS, json: 'rc2/RC296_DEVELOPMENT_CORPUS.json', gz: 'rc2/rc296-development-corpus.jsonl.gz'}
});
