#!/usr/bin/env node
// RC2.1 — the sign-off holdout, on a fresh seed.
//
// Holdout B is not reused. It failed, it is preserved as diagnostic evidence,
// and a holdout an engine has already been remediated against is not a holdout.
//
...
export const HOLDOUT_SEED = 'AUDIT-2026-09-12-C';
export const OUT = 'rc2/HOLDOUT_C.json';
export const OUT_BLIND = 'rc2/holdout-c-blind.jsonl.gz';
export const OUT_FULL = 'rc2/holdout-c-full.jsonl.gz';
 ...
 * touching AUDIT-2026-09-12-C. The real seed is the default, the CLI never
...
export function buildHoldoutC({seed = HOLDOUT_SEED, force = false} = {}) {
  ...
  batch.sessions.forEach((s, i) => {
    ...
    s.questions.forEach((q, k) => {
      n++;
      const itemId = `C-S${i + 1}-${String(k + 1).padStart(2, '0')}`;
  ...
  return {
    report: {
      schema: 'rc21-holdout-v1',
      section: 'RC2.1 sign-off holdout',
      generatedAt: new Date().toISOString(),
      engineVersion: ENGINE_VERSION,
      holdoutSeed: seed,
      previousHoldout: {seed: 'AUDIT-2026-09-12-B', status: 'FAILED_DIAGNOSTIC_HOLDOUT', reused: false},
